...
void menu3_rank(WINDOW* win) {
	...
	if (chdir("..") == 0) {
		getcwd(name, 100);
		mvwprintw(win, 7, 2, "Path is %s", name);
		wrefresh(win);
	}
	...
	if (uflag == 1) { // 사용자 그룹을 얻었을 때
		...
		DIR* dir_info = opendir("."); // ./user 디렉토리 열기
		if (dir_info != NULL) { // NULL값 검사
			struct dirent* entt;
			while ((entt = readdir(dir_info)) != NULL) {
				/*mvwprintw(win, x, 2, "%s", ent->d_name);
				x++;
				wrefresh(win);*/
				if (entt->d_type == DT_DIR && strcmp(entt->d_name, groupid) == 0) {
					group_flag = 1;
				}
			}
			closedir(dir_info);
		...
		if (groupdir != NULL) { // NULL값 검사
			...
			while ((ent = readdir(groupdir)) != NULL) {
				if (ent->d_type == DT_DIR && strcmp(ent->d_name, ".") != 0 && strcmp(ent->d_name, "..") != 0) {
					users_flag = 1;
					mvwprintw(win, 11, 2, "hi");
					wrefresh(win);
					strcpy(rank[i].user_name, ent->d_name);
					mvwprintw(win, 12+i, 2, "%s", rank[i].user_name);
					i++;
				}
			}
			mvwprintw(win, 15, 2, "%d", i);
